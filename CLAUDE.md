...
- **ROLES COMO SKILLS (v3):** Los roles Manager, ProductAgent y QaAutomation1 se ejecutan como skills en el hilo principal, no como subagentes (los agentes de v3 están archivados en `v3/.claude/agents-archive/`). El hilo principal es el Manager (`v3:manager`) y, antes de hacer el trabajo de otro rol, carga su skill: `v3:product-agent` para todo lo de Jira/Xray (PASO 2, reporte de resultados, transiciones) y `v3:qa-automation1` para código, Cypress, Git y PR (PASO 3). Mientras un rol está cargado, se actúa solo dentro de sus responsabilidades y prohibiciones.
