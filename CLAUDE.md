...
2. **Creación en orden correlativo de Test Cases (Xray).** Al crear varios Test Cases del mismo issue, usar `v3/scripts/create-jira-task.js` (ver nota de estado arriba) con el campo `testcaseModels` (array, plural) en vez de invocar el script una vez por Test Case. Internamente (`publishTestCasesBatch` en `v3/scripts/lib/xray.js`) se resuelve primero, de forma secuencial, el `folderId` de cada ruta de carpeta única y el Test Cycle (si corresponde) — son operaciones "buscar o crear" que no son seguras de paralelizar (dos llamadas concurrentes podrían no ver el recurso recién creado y duplicarlo, o directamente fallar contra la API si dos intentan crear la misma carpeta al mismo tiempo). Recién con esos ids ya resueltos, la creación de cada Test Case (+ steps + link + ejecución) se dispara **en secuencia estricta** (await paso a paso, uno completo antes de arrancar el siguiente) — a pedido explícito del usuario (2026-09-13), para que los Test Cases entren en orden correlativo tanto en la Historia como en el Test Repository de Xray, algo que la paralelización previa (`Promise.allSettled`) no garantizaba porque las respuestas de la API podían llegar desordenadas aunque las llamadas se dispararan en orden. Esto prioriza orden por sobre velocidad de creación; si un Test Case falla no aborta el batch (se sigue con los siguientes, misma lección de SCRUM-62), pero cada uno espera a que el anterior termine.
