...
class CheckoutPage {
    ...
    // Verifica que el contenido de la direccion de entrega y facturacion
    // coincide con los datos ingresados durante el registro de la cuenta.
    // Recibe un objeto con: firstName, lastName, address, city, state, zipcode, country, mobile
    verifyAddressContent({ firstName, lastName, address, city, state, zipcode, country, mobile }) {
        const fullName = `${firstName} ${lastName}`

        // Helper interno: valida los campos dentro de una seccion de direccion dada
        const validateSection = ($section) => {
            const sectionText = $section.text()
            expect(sectionText).to.include(fullName)
            expect(sectionText).to.include(address)
            expect(sectionText).to.include(city)
            expect(sectionText).to.include(state)
            expect(sectionText).to.include(zipcode)
            expect(sectionText).to.include(country)
            expect(sectionText).to.include(mobile)
        }

        // Verificar direccion de entrega
        this.deliveryAddressSection.then(validateSection)

        // Verificar direccion de facturacion (billing)
        this.billingAddressSection.then(validateSection)
    }
